import { CheckCircle2, Download, ExternalLink, FileDown, FileUp, Gauge, RefreshCw, RotateCcw } from 'lucide-react'
...
export function SettingsScreen(): React.JSX.Element {
  ...
  return <section className="screen settings-grid">
    <div className="panel settings-card"><div className="panel-title"><h2>App</h2><span className="version-chip">v{info.version}</span></div><div className="settings-rows"><div><span>App version</span><strong>{info.version}</strong></div><div><span>Game data</span><strong>{info.catalogVersion}</strong></div><div><span>Graphics</span><strong className="gpu-state"><Gauge size={15} /> Hardware required</strong></div><div><span>Active GPU</span><strong>{info.gpu.device}</strong></div><div><span>Profile</span><strong className="path-text" title={info.profilePath}>{info.profilePath}</strong></div></div><button className="ghost-button" onClick={() => void window.holodori.app.openProjectPage()}>GitHub project <ExternalLink size={16} /></button></div>
    ...
    <div className="panel settings-card"><div className="panel-title"><h2>Profile backup</h2></div><div className="backup-actions"><button className="action-tile" disabled={busy} onClick={() => void exportProfile()}><FileDown size={24} /><span><strong>Export</strong></span></button><button className="action-tile" disabled={busy} onClick={() => void chooseImport()}><FileUp size={24} /><span><strong>Import</strong></span></button></div></div>

    <div className="panel settings-card attribution"><div className="panel-title"><h2>About</h2></div><p>Unofficial fan project. Game images and names belong to their respective owners.</p><div className="license-line"><span>License</span><strong>MIT</strong></div></div>

    {preview && <Modal title="Replace current profile?" onClose={() => setPreview(null)} footer={<><button className="ghost-button" onClick={() => setPreview(null)}>Cancel</button><button className="primary-button" disabled={busy} onClick={() => void commitImport()}>Replace profile</button></>}><div className="import-file">{preview.fileName}</div><div className="preview-stats"><div><span>Cards</span><strong>{preview.summary.cards}</strong></div><div><span>Active goals</span><strong>{preview.summary.activeGoals}</strong></div><div><span>Inventory units</span><strong>{preview.summary.inventoryUnits.toLocaleString()}</strong></div></div><div className="inline-alert warning">Your current profile will be backed up, then replaced.</div></Modal>}
  </section>
}
